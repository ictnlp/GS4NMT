...
def selu(X):                                                                     
    ...
    return scale*mx.sym.where(condition=X, x=X, y=alpha*(mx.sym.exp(X)-1))

z = np.random.randint(0, 3,(4,4))
z = mx.nd.array(z)
print z.asnumpy()
print orth(z, 4).asnumpy()

def orth(M, dim0_size):
    M = mx.nd.split(M, num_outputs=dim0_size, axis=0, squeeze_axis=False)
    M = [M[i] for i in range(dim0_size)]
    norm_b =M[0]
    w_b = mx.nd.dot(norm_b, norm_b, transpose_b=True)
    
    for i in range(dim0_size - 1):
        t = i + 1
        norm_a = mx.nd.broadcast_div(mx.nd.dot(M[t], norm_b, transpose_b=True), w_b)
        residual = mx.nd.dot(norm_a, norm_b)
        M[t] -= residual
        norm_b = mx.nd.concat(*[norm_b, M[t]], dim=0)
        w_b = mx.nd.concat(*[w_b, mx.nd.dot(M[t], M[t], transpose_b=True)], dim=1)
    return mx.nd.L2Normalization(norm_b, eps=1e-30)        

def initU(M, dim0_size ,eye):
    M = mx.sym.broadcast_add(M*eye, M*(1-eye))
    return orth(orth(M, dim0_size), dim0_size)
    '''
...
def initE(param, dim0_size):
    M = param
    eye = mx.sym.one_hot(indices=mx.sym.arange(start=0, stop=dim0_size, step=1, repeat=1), depth=dim0_size)
    M = mx.sym.repeat(M, repeats=dim0_size, axis=0)
    return M * eye, eye

def Unitry(param1, param2, param3, dim0_size):
    E, eye = initE(param3, dim0_size)
    Q = initU(param1, dim0_size, eye)
    P = initU(param2, dim0_size, eye)
    QP = mx.sym.dot(Q, P)
    #ratio = 100
    #E = ratio * E
...
def initUMatrix(prefix, size):
    param1 = mx.sym.Variable("%s_1_weight"%prefix, shape=(size, size))
    param2 = mx.sym.Variable("%s_2_weight"%prefix, shape=(size, size))
    param3 = mx.sym.Variable("%s_3_weight"%prefix, shape=(1, size))
    U_a, U_b = Unitry(param1, param2, param3, size)
    return U_a, U_b
