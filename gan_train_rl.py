...
class Trainer:
    ...
    def train(self, train_data, name='default', percentage=0.1):
        ...
        for eid in range(wargs.start_epoch, wargs.max_epochs + 1):
            ...
            for k in range(size):
                ...
                param_2 = [LBtensor_to_Str(trgs[1:-1].cpu(),
                                           trgs_m[1:-1].cpu().data.numpy().sum(0).tolist())]
                wlog(bleu(param_1, param_2))

                '''
                for bid in range(batch_srcs_LB.size(1)):
                    onebest_ids = self.try_trans(batch_srcs_LB[:, bid].data,
                                                 batch_refs_LB[:, bid].data)
                    trans_list = numpy_to_str(tc.Tensor(onebest_ids),
            ...
            for bid in range(batch_count):
                ...
                for i in range(2):
                    ...
                    p_y_hyp = tc.sum(tc.log(p_y_hyp) * hyps_mask , 0) / hyps_mask.sum(0)
                    ...
                    rho = rho / tc.sqrt(D_a * D_b)
                    #print D_a.data[0], 'da'
                    #loss = tc.mean((-p_y_hyp) * bleus)
                    loss = -rho

                    (1 * loss).backward()
                    wlog('Mean BLEU: {}, rho: {}'.format(avg_bleu, rho.data[0]))
