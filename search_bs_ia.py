...
class NBS(Func):
    ...
    def beam_search_comb(self, s_tensor):
        ...
        s_tm1, hs_tm1, uh_tm1 = self.model.init(s_tensor, test=True)
        # s_tm1: 1x512
        slen, enc_size, aln_size = hs_tm1.size(0), hs_tm1.size(2), uh_tm1.size(2)

        maxlen = self.maxlen
        hyp_scores = np.zeros(1).astype('float32')
        if wargs.dec_layer_cnt > 1: s_tm1 = [s_tm1] * wargs.dec_layer_cnt

        init_beam(self.beam, cnt=maxlen, hs0=hs_tm1, s0=s_tm1, detail=True)

        for t in range(1, self.maxlen + 1):
            if (t - 1) % 10 == 0: debug(str(t - 1))

            prevb = self.beam[t - 1]
            preb_sz = len(prevb)

            # (slen, 1, enc_hid_size) -> (slen, enc_hid_size) -> (slen, preb_sz, enc_hid_size)
            hs_tm1 = tc.stack(tuple([b[1].squeeze(1) for b in prevb]), dim=1)
            uh_tm1 = self.model.gen_uh(hs_tm1)

            # batch states of previous beam, (preb_sz, 1, nhids) -> (preb_sz, nhids)
            s_tm1 = tc.stack(tuple([b[-3] for b in prevb]), dim=0).squeeze(1)
            #c_im1 = [tc.stack(tuple([prevb[bid][1][lid] for bid in range(len(prevb))])
            #                 ).squeeze(1) for lid in range(len(prevb[0][1]))]
            y_tm1 = [b[-2] for b in prevb]

            #c_t, s_t = self.decoder.step(c_tm1, enc_src, uh, y_tm1)
            a_t, s_t, y_tm1, alpha_ij = self.decoder.step(s_tm1, hs_tm1, uh_tm1, y_tm1)
            hs_tm1 = self.decoder.write_attention(alpha_ij, s_t, hs_tm1)

            # (preb_sz, out_size)
            # logit = self.decoder.logit(s_t)
            logit = self.decoder.logit(s_t, y_tm1, a_t)
            ...
            hs_tm1 = [hs_tm1[:, idx, :].unsqueeze(1) for idx in prevb_id]
            #for b in zip(costs, batch_ci, word_indices, prevb_id):
            for b in zip(costs, hs_tm1, s_t[tp_bid], word_indices, prevb_id):
                if b[-2] == const.EOS:
                    if wargs.with_norm:
                        self.translations.append(((b[0] / t), b[0]) + b[-2:] + (t, ))
                    else:
                        self.translations.append((b[0], ) + b[-2:] + (t,))
                    if len(self.translations) == self.k:
                        # output sentence, early stop, best one in k
                        debug('early stop! see {} samples ending with EOS.'.format(self.k))
                        avg_bp = format(self.locrt[0] / self.locrt[1], '0.3f')
                    ...
                    self.locrt[0] += (b[-1] + 1)
                    self.locrt[1] += 1
                    self.beam[t].append(b)
            debug('beam {} ----------------------------'.format(t))
            for b in self.beam[t]:
                debug(b[0:1] + b[-2:])    # do not output state
            hyp_scores = np.array([b[0] for b in self.beam[t]])
