    ...
    def hyps_padding_dist(self, B, hyps_L, y_maxL, p_y_hyp):
        ...
        for bid in range(B):
            ...
            if hyp_L < y_maxL:
                ...
                pad = pad.unsqueeze(-1).expand((pad.size(0), one_p_y_hyp.size(-1)))
    ...
    def train(self, train_data, name='default'):
        ...
        for eid in range(wargs.start_epoch, wargs.max_epochs):
            ...
            for bid in range(batch_count):
                ...
                wlog('Train Discrimitor .......... {} ... {}'.format(bid, name))
                for j in range(self.n_critic):
                    ...
                    loss_D, w_loss_D = self.distance(p_y_gold, p_y_hyp, hyps_mask,
                    ...
                    #print self.nmtModel.ha.weight.grad.data
                    #print self.nmtModel.ha.weight
                    #print 'after...........'
                    self.optim.step()
                    #print self.nmtModel.ha.weight
                    del o1, p_y_gold, p_y_hyp, hyps_mask
                ...
                #self.optim.zero_grad()
                #p_y_hyp2, hyps_mask = self.beamsearch_sampling(srcs, srcs_m,
                ...
                #print hyps_mask.size()
                # padding hyps
                #loss_G = self.distance(p_y_gold, p_y_hyp2, hyps_mask, type='KL')
                ...
                for i in range(5):
                    ...
                    bleus = Variable(tc.Tensor(bleus), requires_grad=False)
                    if wargs.gpu_id: bleus = bleus.cuda()
                    loss = tc.sum((-log_p_y_hyp) * bleus)
                    #loss = tc.sum((-log_p_y_hyp))
                    ...
                    del hyps, hyps_mask, o_hyps, p_y_hyp, log_p_y_hyp, bleus, loss
